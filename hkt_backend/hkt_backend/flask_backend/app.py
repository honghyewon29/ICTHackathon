...
app.secret_key = 'your-super-secret-key'  # 🔐 이 줄 추가!
app.register_blueprint(chat_blueprint, url_prefix="/api/chat")
app.register_blueprint(rag_blueprint, url_prefix="/api/rag")
app.config['JWT_SECRET_KEY'] = 'your-super-secret-key'  # 이거 꼭 있어야 함!
jwt = JWTManager(app)  # <-- 이거 필수!!!!!
verification_codes = {}  # 🔥 전역 선언 필수!
...
def create_lost_item():
    ...
    try:
        ...
        return jsonify({'message': '분실물 등록 완료'}), 200
